...
def get_mongo_client():
    ...
    if "MONGO_CONNECTION_STRING" not in st.secrets:
        st.error("CRÍTICO (user_management_db.py): 'MONGO_CONNECTION_STRING' não configurada nos segredos do Streamlit Cloud.")
        print("CRITICAL_ERROR_LOG: MONGO_CONNECTION_STRING not found in Streamlit Cloud secrets.")
        return None
    ...
    try:
        ...
        client = MongoClient(
            ...
            serverSelectionTimeoutMS=25000, # Aumentado um pouco para ambientes de nuvem
        ...
        print(f"MONGODB_TIMEOUT_ERROR_LOG: {sste}")
        st.info("Verifique 'Network Access' no MongoDB Atlas (deve permitir acesso de qualquer lugar para Streamlit Cloud) e os logs do aplicativo.")
        return None
    except Exception as e:
        st.error(f"ERRO GERAL AO CONECTAR AO MONGODB (user_management_db.py): {e}")
        print(f"MONGODB_GENERAL_CONNECTION_ERROR_LOG: {e}, Type: {type(e)}")
        return None
...
def get_users_collection():
    ...
    try:
        ...
        print(f"GET_COLLECTION_ERROR_LOG: {e}")
        return None
...
def fetch_all_users_for_auth():
    ...
    if users_collection is None:
        print("WARN_LOG (user_management_db.py - fetch_all_users): Coleção de usuários não disponível ao buscar.")
    ...
    try:
        ...
        print(f"FETCH_USERS_DB_ERROR_LOG: {e}")
        return default_credentials
        
    credentials = {"usernames": {}}
    for user in users_from_db:
        if not all(key in user for key in ["username", "name", "hashed_password", "role"]):
            print(f"WARN_LOG (user_management_db.py): Usuário com dados incompletos (ID: {user.get('_id', 'N/A')}).")
            continue
...
def get_user_role(username: str):
    ...
    try:
        ...
        print(f"GET_ROLE_ERROR_LOG: {username}, {e}")
        return None
...
def add_user(username: str, name: str, email: str, plain_password: str, role: str):
    ...
    try:
        ...
        print(f"ADD_USER_ERROR_LOG: {username}, {e}")
        return False

# ... (O restante das funções CRUD: update_user_details, delete_user, 
# update_user_password_by_admin, update_user_password_self, get_all_users_for_admin_display
# permanecem como na última versão completa que forneci, com as checagens "if users_collection is None:")
...
def update_user_details(username: str, name: str, email: str, role: str):
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            st.info(f"Nenhuma alteração nos dados do usuário '{username}'.")
            return True 
        else:
...
def update_user_password_by_admin(username: str, plain_password: str):
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            st.warning(f"Senha do usuário '{username}' não alterada.")
            return False
    except Exception as e:
