...
def update_user_details(username: str, new_name: str, new_email: str, new_role: str):
    ...
    update_fields = {}
    # Apenas adiciona ao $set se o valor não for None ou vazio (decisão de design)
    # Ou, se quiser permitir limpar campos, ajuste esta lógica.
    ...
    if not update_fields: # Se nenhum campo foi alterado/fornecido
        st.info(f"Nenhum dado novo fornecido para o usuário '{username}'. Nenhuma alteração realizada.")
        return True # Considera sucesso pois não houve erro, apenas nenhuma ação.
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            return True
        else: # matched_count > 0 mas modified_count == 0
