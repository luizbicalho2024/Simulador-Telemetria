...
def get_fipe_precos_por_modelo(tipo_veiculo, codigo_marca, codigo_modelo):
    """Busca TODOS os anos/versões de um modelo específico."""
...
# Função para formatar e exibir resultados
...
with tab_fipe:
    st.info("""
    **Como funciona:**
    1.  **Busca por Nome:** Pesquise primeiro em nossa base de dados local.
    2.  **Busca Guiada (API):** Se não encontrar, use a busca guiada para consultar a FIPE. Ao selecionar um modelo, **todos os seus anos e versões serão salvos automaticamente** em nosso banco, agilizando futuras consultas.
    """)
    ...
    tipo_veiculo = col_tipo.selectbox(
        "Tipo de Veículo", ["carros", "motos", "caminhoes"], index=None, placeholder="Selecione...", key="fipe_tipo"
    )
    ...
    if tipo_veiculo:
        ...
        if marca_selecionada:
            ...
            modelo_selecionado_nome = col_modelo.selectbox(
                "Modelo", list(modelos_dict.keys()), index=None, placeholder="Selecione...", key="fipe_modelo"
            )
                ...
                if st.button("Consultar e Salvar Dados do Modelo", type="primary", use_container_width=True):
                    ...
                    with st.spinner(f"Buscando todas as versões de '{modelo_selecionado_nome}' na API FIPE e salvando no banco..."):
                        ...
                        if dados_completos_modelo:
                            ...
                            st.error("Não foi possível obter os dados para este modelo na API externa.")

# --- ABA DE CONSULTA CNPJ ---
with tab_cnpj:
